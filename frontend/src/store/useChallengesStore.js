...
export const useChallengesStore = create((set) => ({
  ...
  addChallenge: async (data) => {
    ...
    try {
      const response = await axiosInstance.post(
        ...
        {
          ...data,
          status: true, // Set status to true so it can be assigned immediately
        },
        {
          headers: {
      ...
      set((state) => ({
        challenges: [...state.challenges, response.data.challenge_data],
      }));
      toast.success("Challenge added successfully");
    } catch (error) {
      const errorMessage =
        error.response?.data?.error || "Failed to add challenge";
  ...
  getChallenges: async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to get challenges";
      toast.error(errorMessage);
    } finally {
  ...
  assignChallenge: async (challengeId, teamId) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to assign challenge";
      toast.error(errorMessage);
      throw error;
    } finally {
